...
<head>
  ...
  <style>
    ...
    .bubble.consultant{background:#0070b8;color:#fff;margin-left:auto;border-bottom-right-radius:8px 20px;}
    .bubble.client{background:#e3f2fd;color:#14456c;border-bottom-left-radius:8px 20px;}
    .bubble .msg-time{display:block;font-size:.91em;color:#ddeafb;margin-top:2px;float:right;margin-left:8px;}
    .bubble.client .msg-time{color:#7e98b6;}
...
function loadClientInbox() {
  ...
  // Load all clients this consultant has ever had an appointment with
  $.getJSON('../api/appointments.php?action=list', function(appts) {
      ...
      appts.forEach(a => {
        if(!seen[a.client_id]) {
          ...
          clients.push({
            ...
            pic: a.client_pic||"https://ui-avatars.com/api/?name="+encodeURIComponent(a.client_name)+"&background=0070b8&color=fff"
          });
        }
      });
    }
    // Load messages for thread previews
    $.getJSON('../api/messages.php?action=inbox', function(resp){
      ...
      if(!clients.length){$('#conv-list').html('<div class="text-center text-muted p-4">No conversations.</div>');return;}
...
function loadThread(user_id, user_name) {
  ...
  $.getJSON('../api/messages.php?action=thread&user_id='+user_id, function(resp){
    ...
    $('#message-list').html(resp.messages.map(m=>{
      ...
      let cls = isMe?'consultant':'client';
      let time = m.sent_at.substr(11,5);
      return `<div class='bubble ${cls}'><span class='msg-meta'>${isMe?'You':'Client'}</span> ${m.content}<span class='msg-time'>${time}</span></div>`;
    }).join(''));
